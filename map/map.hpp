 ...
 //#include "map_ite.hpp"
  namespace ft {
    template <
      ...
      class Alloc = std::allocator < ft::pair <
      const Key, T > > /*   */
        >
        class map {
          ...
          typedef typename ft::pair <
          const key_type, mapped_type > value_type;
          ...
          //// iterator 
          typedef ft::tree_iterator < Key,
          const Node,
            const value_type, Alloc, AVL < Key, T, value_type, Alloc, Compare > > const_iterator;
          typedef ft::tree_iterator < Key,
          Node,
          value_type,
          Alloc,
          ...
          bool > {
            ...
            public: bool operator()(const value_type & x,
          ...
          T,
          Compare,
          Alloc > & operator = (const map < Key, T, Compare, Alloc > & x) {
            if (this == & x)
              return ( * this);
            this -> clear();
          ...
          pair < iterator,
          iterator > equal_range(const key_type & k) {
          ...
          public: friend bool operator == (const map & lhs,
            const map & rhs) {
